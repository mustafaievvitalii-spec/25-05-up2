...
import { Instagram, MessageCircle, Send, Smartphone, Timer, Zap } from 'lucide-react';
  ...
  return (
    <main className="min-h-screen bg-[#000000] text-white">
      ...
      <div className="fixed bottom-[max(1rem,env(safe-area-inset-bottom))] right-4 z-[120] flex flex-col gap-3 sm:right-6">
        {[{ label: 'Написати у WhatsApp', href: WHATSAPP_URL, icon: MessageCircle }, { label: 'Написати у Viber', href: VIBER_URL, icon: Smartphone }].map((item) => (
          <motion.a key={item.label} href={item.href} target="_blank" rel="noopener noreferrer" title={item.label} whileHover={{ y: -3, scale: 1.04 }} animate={{ y: [0, -3, 0] }} transition={{ duration: 2.4, repeat: Infinity }} className="group relative flex h-14 w-14 items-center justify-center rounded-full border border-[#D4FF00]/60 bg-[#0f0f0fcc] text-[#D4FF00] shadow-[0_10px_30px_rgba(0,0,0,0.55)] backdrop-blur-xl">
      ...
      <section className="mx-auto max-w-7xl px-6 pb-10 pt-16 lg:px-10">
        <motion.div {...fadeInUp} className="rounded-3xl border border-neutral-800 bg-[#0A0A0A] p-8 lg:p-10">
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <a className={socialLinkClasses} href={TIKTOK_URL} target="_blank" rel="noopener noreferrer" aria-label="TikTok"><Send className="h-5 w-5" /></a>
      ...
      <section className="mx-auto max-w-7xl px-6 pb-10 lg:px-10"><motion.div {...fadeInUp} className="rounded-3xl border border-neutral-800 bg-[#0A0A0A] p-7"><h2 className="text-3xl font-bold">Як проходить процедура</h2><div className="mt-7 grid gap-4 lg:grid-cols-6">{steps.map((step, idx) => <div key={step} className="group relative rounded-2xl border border-neutral-800 bg-[#111111] p-4 transition hover:border-[#D4FF00]/50 hover:shadow-[0_0_24px_rgba(212,255,0,0.16)]"><div className="mb-3 flex items-center gap-2 text-[#D4FF00]"><Send className="h-4 w-4" /><span className="text-xs font-semibold">Крок {idx + 1}</span></div><p className="text-sm text-zinc-200">{step}</p></div>)}</div></motion.div></section>

      <section className="mx-auto max-w-7xl px-6 pb-10 lg:px-10"><h2 className="text-3xl font-bold">Необхідні документи</h2><div className="mt-5 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-12">{docs.map((doc, idx) => <motion.div key={doc} {...fadeInUp} transition={{ duration: 0.5, delay: idx * 0.06 }} className={`group rounded-2xl border border-neutral-800 bg-[#111111] p-5 ${idx === 0 ? 'lg:col-span-6' : 'lg:col-span-3'}`}><div className="mb-3 inline-flex rounded-full border border-neutral-700 bg-black p-2"><MessageCircle className="h-5 w-5 text-[#D4FF00]" /></div><p className="text-zinc-100">{doc}</p></motion.div>)}</div></section>
